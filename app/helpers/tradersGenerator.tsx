...
export function generateTraders(): Trader[] {
	const traders: Trader[] = [
		...
		{
			accountName: "Syperek",
			characterName: "我在這裡撫摸我的雞巴我現在在我的雞巴上塗了乳液",
			type: TraderType.Scammer,
		...
		{ accountName: "Ktz", characterName: "nutDeveloper", type: TraderType.Honest, region: Region.English },
	];
	for (let i = 0; traders.length < 150; i++) {
		let newTrader = generateRandomTrader();
		const nameExists = traders.some(
			(trader) => trader.accountName === newTrader.accountName || trader.characterName === newTrader.characterName
		);
		if (!nameExists) {
...
export default function generateTradeListings(item: string, traders: Trader[]): TradeItem[] {
	...
	randomTraders.forEach((trader) => {
		...
		if (item == "Vivid Crystallised Lifeforce") defaultTradeItem = generateTradeItemLifeForce(item, trader);
		else defaultTradeItem = generateTradeItem(item, itemPrice, trader);
...
function generateLifeForcePrice(type: TraderType): number {
	...
	if (type == TraderType.OilPrince) price = 10;
	if (type == TraderType.Scammer) price = 1;
	if (type == TraderType.PriceFixer) price = 1;
...
function generateLifeForceStock(type: TraderType): number {
	...
	if (type == TraderType.PriceFixer) stock = 6500;
	if (type == TraderType.OilPrince || type == TraderType.Scammer) stock = 6000;
	return stock;
}
...
function generateItemPrice(defaultPrice: number, type: TraderType): number {
	...
	if (defaultPrice > 3) {
		...
		if (type == TraderType.Scammer) price = defaultPrice;
		if (type == TraderType.PriceFixer || type == TraderType.OilPrince) price = defaultPrice - 0.5;
	} else {
		if (randomNum < 0.7) {
			price = defaultPrice;
		} else if (randomNum < 0.8) {
...
function generateItemStock(type: TraderType): number {
	...
	if (type == TraderType.PriceFixer || type == TraderType.Scammer) stock = 1;
	if (type == TraderType.OilPrince) stock = Math.floor(Math.random() * 8) + 10;
	return stock;
}
...
function generateRandomTrader(): Trader {
	...
	const containsKoreanCharacter = /[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]+/.test(
		accountName
	);
	...
	if (containsJapaneseCharacter) region = Region.日本語;
	if (containsKoreanCharacter) region = Region.한국어;
	if (containsRussianCharacter) region = Region.Русский;
	if (containsThaiCharacter) region = Region.ไทย;
	return {
		accountName,
		characterName,
